...
new #[Title('Nowe zgłoszenie')] class extends Component {
    use WithFileUploads;
    ...
    public function save(): void
    {
        ...
        session()->flash('success', __('Ticket #:id submitted! We\'ll get back to you soon.', ['id' => $ticket->id]));
...
<div>
    ...
    <div class="mb-8">
        ...
        <flux:heading size="xl" level="1">{{ __('Submit a Support Ticket') }}</flux:heading>
        <flux:subheading>{{ __('Describe your IT issue and our team will respond as soon as possible.') }}</flux:subheading>
    </div>
        ...
        <form wire:submit="save" class="space-y-6" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                ...
                <flux:field>
                    <flux:label>
                        {{ __('Problem Category') }}
                        <flux:badge variant="outline" size="sm" class="ml-1">{{ __('required') }}</flux:badge>
                    </flux:label>
                    <flux:select wire:model="category" id="category" :placeholder="__('Select a category…')">
            ...
            <flux:field>
                ...
                <flux:textarea
                    ...
                    id="description"
                    rows="6"
                    :placeholder="__('What happened? Since when? Which software or hardware is affected?')"
                />
                <flux:description>{{ __('Minimum 10 characters. Be as specific as possible.') }}</flux:description>
            ...
            <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                ...
                <flux:field>
                    <flux:label>
                        {{ __('Hardware Name / ID') }}
                        <flux:badge variant="outline" size="sm" class="ml-1">{{ __('optional') }}</flux:badge>
                    </flux:label>
                    <flux:input
                        wire:model="hardware_name"
                        id="hardware_name"
                        type="text"
                        :placeholder="__('e.g. PC-042, Printer HP LaserJet')"
                    />
                    <flux:description>{{ __('If hardware-related, provide the asset name or ID.') }}</flux:description>
            ...
            <div class="flex items-center justify-end gap-3 border-t border-zinc-200 pt-4 dark:border-zinc-700">
                ...
                <flux:button type="submit" variant="primary" wire:loading.attr="disabled" wire:target="save">
                    <span wire:loading.remove wire:target="save">{{ __('Submit Ticket') }}</span>
